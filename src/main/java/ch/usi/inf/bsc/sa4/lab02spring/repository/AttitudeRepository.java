...
public interface AttitudeRepository extends MongoRepository<LevelAttitude, String>, AttitudeStatisticsRepository {
    ...
    /// Returns all user attitudes of a specific level. So far, counting
    /// likes/dislikes outside of DB.
    /// 
    ...
    /// Find a single attitude by level id and user id.
    /// 
    /// @param levelId the level id
    /// @param userId  the user id
    /// @return optional attitude
    @Query("{ 'level.id': ?0, 'user.id': ?1 }")
    Optional<LevelAttitude> findByLevelIdAndUserId(String levelId, String userId);
