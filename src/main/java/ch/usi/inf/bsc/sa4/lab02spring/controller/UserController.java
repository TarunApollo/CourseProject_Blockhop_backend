...
public class UserController {
    ...
    ///         played levels count, completed levels count, and list of created
    ...
    /// @return a 200 OK with the newly created user dto, otherwise return the
    ///         existing user dto information
    @GetMapping(path = "/me")
    public ResponseEntity<UserDTO> index(Authentication authentication) {
        ...
        return optUser.map(user -> ResponseEntity.ok(new UserDTO(user)))
                .orElseGet(() -> ResponseEntity
                        .ok(new UserDTO(this.userService.createUser(new CreateUserDTO(eduId, fullName)))));
    }
}