...
@SuppressWarnings({ "NullAway.Init", "PMD.TooManyMethods", "PMD.UseConcurrentHashMap" })
...
public class Level {
    ...
    ///               published is set to false, width to 256, height to 14, and
    ///               clearCondition with NoCondition and 0 target value objectLayer,
    ///               and worldLayer are initialized as empty maps.
    ...
    @PersistenceCreator
    public Level(final User creator, final String title, final String description, final boolean published,
            final ClearCondition clearCondition, final Map<Position, GameObject> objectLayer,
    ...
    /// @param objectLayer a map of positions to game objects to copy into the object
    ///                    layer
    /// @param worldLayer  a map of positions to ground objects to copy into the
    ///                    world layer
    ///
    private Level(final String title, final String description, final User creator,
            final Map<Position, GameObject> objectLayer, final Map<Position, GroundObject> worldLayer,
    ...
    /// @return a new Level with the same description, clearCondition, objectLayer,
    ///         and worldLayer as this level, but with the given creator, title, and
    ...
    /// remove a game object from the objectLayer(why on earth do we call it the
    /// object layer if they are all objects??)
    ...
    /// 
    /// @spec.modifies this.
    /// @spec.effects marks this level as published if the given user owns the level
    ///               and the level is already marked as publish eligible.
    ...
    /// @param userId the ID of the user requesting to invalidate publish
    ///               eligibility.
    /// @throws ForbiddenUserException if the given user is not the owner of
    ///                                this level.
    ...
    /// Set this level unpublished. This operation is idempotent.
    /// 
    /// @spec.modifies this.
    ...
    public void setClearCondition(final ClearCondition clearCondition) {
        this.clearCondition = clearCondition;
    }
    ...
    /// Ensures that the provided object layer contains at most one start flag and
    /// one exit door.
    /// 
    ...
    /// Ensures that the current object layer is valid for publishing. A published
    /// level must contain exactly one start flag and exactly one exit door.
    /// 
    /// @throws ForbiddenLevelActionException if publish requirements
    ///                                       aren't satisfied
