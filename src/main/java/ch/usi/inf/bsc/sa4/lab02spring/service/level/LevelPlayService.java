...
public class LevelPlayService {
    ...
    /// Resolves tile identifiers to catalog entries.
    ...
    /// @param tileCatalogService  resolves tile ids
    ...
    /// @spec.effects validates the attempt against the level state; if the level is
    ///               unpublished and owned by the given user and the attempt is
    ///               successful, marks the level as publish eligible; records the
    ///               attempt through the attempt service.
    ...
    /// @throws LevelNotFoundException        if no level with the given levelId
    ///                                       exists
    ...
    /// Returns a catalog-enriched map representation of a playable level.
    ///
    /// @spec.requires user and levelId are not null.
    /// @spec.effects loads the requested level, verifies that the given user is
    ///               allowed to play it, and converts the level into a
    ///               frontend-consumable map structure using the tile catalog.
    ...
    public Map<String, Object> getPlayableMap(final User user, final String levelId) {
        ...
        return LayerToTiledMapConverter.convertPipeline(level, this.tileCatalogService);
    }
}
