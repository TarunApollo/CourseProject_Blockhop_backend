...
public class LevelAttitudeService {
    ...
    /// Sets or updates a user's attitude towards a level.
    /// If the user already has an attitude for this level, it will be updated;
    /// otherwise, a new attitude record is created.
    ///
    /// @spec.requires userId and levelId are not null.
    /// @spec.effects creates a new LevelAttitude or updates an existing one in the repository.
    ...
    public LevelAttitude setAttitude(final String userId, final String levelId, final LevelAttitudeType attitude) {
        ...
        // Find existing attitude, if any
        final List<LevelAttitude> existingAttitudes = this.attitudeRepository.findByLevel(level).stream()
                .filter(a -> a.getUser().getId().equals(userId))
                .toList();

        if (!existingAttitudes.isEmpty()) {
            // Update existing attitude
            final LevelAttitude existingAttitude = existingAttitudes.get(0);
            existingAttitude.setAttitude(attitude);
            return this.attitudeRepository.save(existingAttitude);
        } else {
            // Create new attitude
            final LevelAttitude newAttitude = new LevelAttitude(user, level, attitude);
    ...
    ///
    /// @spec.requires userId and levelId are not null.
    /// @spec.modifies removes the user's attitude record for the level from the repository if present.
    /// @spec.effects deletes the attitude if found; does nothing if no attitude exists.
    /// @param userId the unique identifier of the user
    /// @param levelId the unique identifier of the level
    /// @throws UserNotFoundException if the user does not exist
    /// @throws LevelNotFoundException if the level does not exist
    public void deleteAttitude(final String userId, final String levelId) {
        ...
        final Optional<LevelAttitude> attitude = this.attitudeRepository.findByLevel(level).stream()
                .filter(a -> a.getUser().getId().equals(userId))
                .findFirst();

        if (attitude.isPresent()) {
            this.attitudeRepository.deleteById(attitude.get().getId());
        }
    }
}
